function []=POSTmain(f2,nnSuperSet,DSuperSet,bcSuperSet,pathANN,flag_region_dom,flag_optim_dom,flag_aqi_dom,...
     ...
     for indexSea = 1:size(nomesea,2), %available season
         for j = 1:size(pathANN(1).ANNs,1), %aqis
            ...
            if (isequal(strtrim(pathANN(indexSea).ANNs(j,:)),'-999')==0)
                ...
                [~,~,aqi_val_CLE]=POSTcompute_aqi(sSet(1).X,nnSuperSet(indexSea).nnSet(j),...
                    DSuperSet(indexSea).DSet(j),bcSuperSet(indexSea).bcSet(j),sSet(1).X,flag_region_dom);
                ...
                [emi_low,emi_high,aqi_val]=POSTcompute_aqi(sSet(indexPareto).X,nnSuperSet(indexSea).nnSet(j),...
                    DSuperSet(indexSea).DSet(j),bcSuperSet(indexSea).bcSet(j),sSet(1).X,flag_optim_dom);
...
%FUNCTION TO COMPUTE PARETO CURVE
    function [cost_final_over_cle,aqi_final]=POSTparetocurve(sSet,budgetCO2,extCost_morb,extCost_yoll,paretoloop)
        ...
        fclose(fid);
        dlmwrite(file,res,'-append','roffset', 0);
    end
...
%FUNCTION TO COMPUTE EMISSIONS AND COSTS TABLES
    function []=POSTcomputeTables(x,index,outdir,pathOCM,CLE,global_data)
        ...
        dlmwrite(nomefile,results,'-append','roffset', 0);
        ...
        %'remSo2',delim,
        fclose(fid);
        dlmwrite(nomefile,emicosttab_msat_tot,'-append','roffset', 0);
        ...
        fclose(fid);
        dlmwrite(nomefile,emicosttab_m_tot,'-append','roffset', 0);
...
%MOD20160531ET
    function [emi_low,emi_high,aqi_val2]=POSTcompute_aqi(x,NN,DD,BCset,x_CLE,AQIdomFILTER)
        %MOD20160531ET
        ...
        %compute aqi related to the CLE solution
        %define low emissions matrix (to be used for emission computation)
        %MOD20160531ET
        ...
        %UNIBS(ET)20131001 - keep only cells in optim domain
        emi_low=emi_low((flag_region_dom==1 | flag_region_dom==2),:);
        ...
        if areal_point==0
            ...
            BCemi=BCset.emi_bc(AQIdomFILTER==1 | AQIdomFILTER==2,:);
            BCconc=BCset.conc_bc(AQIdomFILTER==1 | AQIdomFILTER==2,:);
            flag_opt_filt=AQIdomFILTER(flag_region_dom==1 | flag_region_dom==2,1);
...
if strcmp(NN.Class,'Delta')==0
        ...
        if strcmp(pathAR,'-1')==0
            ...
            %UNIBS(ET)20131002
            AR(flag_region_dom==0,:)=[];                  %remove cells outside flag_region_dom
        ...
        switch flag_reg_net
            case 0
                 %20140404 and 20140612(ET) - Adapted to the new linear models
                    ...
                    aqi2=NN.net'*[input_rete TMPnet]';

            case 1
                %consider linear or nonlinear model cases
                %run network
                ...
                aqi2=((output_rete_norm2-ymin)*G*(xmax-xmin))/(ymax-ymin)+xmin+((xmax-xmin)*(-ymin+offst)/(ymax-ymin));
%                 NN.ps_target.no_change=0; %to be compatible among different matlab versions
%                 aqi2=mapminmax('reverse',output_rete_norm2,NN.ps_target);
                
                %check for results outside bounds - force to be inside bounds
                %check for results outside bounds - force to be inside bounds
                aqi2(find(aqi2<xmin))=xmin;
                aqi2(find(aqi2>xmax))=xmax;
                %%%
...
%FUNCTION TO COMPUTE EMISSIONS AND AQIS MAPS
    function []=POSTcomputeMaps(livelli,map,nomefile,aqilab,indexPareto,outdir,...
        ...
        aqi(isnan(aqi))=-999;
        dlmwrite(file,aqi,'-append','roffset', 0,'precision','%6.2f');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function [extCost_tot_morb,extCost_tot_yoll,resp_prob,card_prob,yoll_prob]=POSTexternalcosts(aqi_val,...
        ...
        full_aqi(flag_optim_dom==1 | flag_optim_dom==2)=aqi_val;
        full_aqi=full_aqi';
        ...
        yoll_prob=impacts(optfilter,14)./(pop(optfilter).*pop30_90(optfilter))*12*60; %in monhts, over 60 years of life

function [ finalInfo  ] = quadrant_Finalize( finalizeJobInfo, prepareDataInfo, intermediateResult, precursors, indicators, ...
...
%nameDirOut=strcat('./output/',prepareDataInfo.domain,'/',prepareDataInfo.aqiFil,...
%    '/absDel_',int2str(prepareDataInfo.absDel),'/arealPoint_',int2str(prepareDataInfo.arealPoint),...
%    '/',nametest,'/rf_',int2str(prepareDataInfo.rf),'-modTyp',int2str(prepareDataInfo.typeOfModel),...
%    '-modVar',int2str(prepareDataInfo.modelVariability),'-pca',int2str(prepareDataInfo.pcaFlag),'/');
%mkdir(nameDirOut);
%nameRegFile=strcat(nameDirOut,'regression.mat');

Prec2=zeros(ny+intermediateResult.rad(1)*2,nx+intermediateResult.rad(1)*2,size(precursors,3),size(precursors,4));
Prec2(intermediateResult.rad+1:end-intermediateResult.rad,intermediateResult.rad+1:end-intermediateResult.rad,:,:)=precursors(:,:,:,:);
PrecOld=precursors;
precursors=[];
precursors=Prec2;
...
%load(strcat('./input/',prepareInfo.domain,'/flagRegioMat.mat'));
% load poland_flag

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% IDENTIFICATION BEGIN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%s%%%%%%%%%%%%%%%%%%%%%%%%%%
if exist(nameRegFile,'file')==0
    ...
    yMin=NaN(ny,nx);
    yMax=NaN(ny,nx);
    bInt=NaN(ny,nx,prepareInfo.nPrec,2);
...
%                 
                                ...
                                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                PrecDummyQuad=squeeze(precursors(ir:ir+intermediateResult.rad+prepareInfo.rad,ic:ic+intermediateResult.rad+intermediateResult.rad,intermediateResult.Ide,:));
...
for indVal=1:size(intermediateResult.Val,2)
    
    %loop on scenarios
    iSc=intermediateResult.Val(indVal);
    disp(strcat('Validating on the independent scenario n: _',int2str(iSc)));
...
finalInfo.finalResult=finalResult;

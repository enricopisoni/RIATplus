...
function []=POSTmain(f2,nnSuperSet,DSuperSet,pathANN,flag_optim_dom,flag_aqi_dom,...
...
%define number of optimization cells
...
%MM20150527 - create an xml external file for this data
                ...
                if j==1 && indexSea==1
                    ...
                    if extpop==-999
                        ...
                        [extCost_tot_morb,extCost_tot_yoll, resp_prob,card_prob,yoll_prob]=...
                            POSTexternalcosts(aqi_val,extpop,extoth,pop,flag_optim_dom);
                        ...
                        POSTcomputeMaps(0:1:10,yoll_prob,...
...
%FUNCTION TO COMPUTE EMISSIONS AND AQIS VALUES PER CELL
    function [emi_low,emi_high,aqi_val2]=POSTcompute_aqi(x,NN,DD)
        ...
        emi_low(length(flag_optim_dom),6)=0;
        emi_high(length(flag_optim_dom),6)=0;
        
        %loop over cells to create emissions file
        for i=1:length(flag_optim_dom)
            
            if flag_optim_dom(i)==0
                %case of cells without optimization domain
                %emission order: NOX, COV, NH3, PM10, PM25, SO2.
                emi_low(i,:)=base_emi_low(i,:);
                emi_high(i,:)=base_emi_high(i,:);
                
            elseif (flag_optim_dom(i)==1 | flag_optim_dom(i)==2)
        ...
        %keep only optimization cells
        emi_low=emi_low(find(flag_optim_dom==1 | flag_optim_dom==2),:);
        emi_high=emi_high((flag_optim_dom==1 | flag_optim_dom==2),:);
...
%FUNCTION TO COMPUTE EMISSIONS AND AQIS MAPS
    function []=POSTcomputeMaps(livelli,map,nomefile,aqilab,indexPareto,outdir,...
        ...
        if detailResults==1
            figure;
            POSTcomputeMapsRead(livelli,aqi,1000,nomefile)
            hold on
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function [extCost_tot_morb,extCost_tot_yoll,resp_prob,card_prob,yoll_prob]=POSTexternalcosts(aqi_val,...
            pathextpop,pathextoth,pop,flag_optim_dom)
        % 	- "external_cost_pop_data"contains
        % 		% pop per age, in % (9 columns: 0-5 6-10 11-14 15-19 20-24 25-29 30-59 60-64 <65)
        % 		% mortality rate per age (10-18 columns: 0-5 6-10 11-14 15-19 20-24 25-29 30-59 60-64 <65)
        % 		% asmathics rate in % (19 column)
        ...
        full_aqi(1:length(flag_optim_dom))=0;
        %         full_aqi(flag_optim_dom==1)=aqi_val;
        full_aqi(flag_optim_dom==1 | flag_optim_dom==2)=aqi_val;
        full_aqi=full_aqi';
        
        %cut extpop data, if necessary (as in RIAT).
        extpop(length(flag_optim_dom)+1:end,:)=[];
        ...
        optdom=find(flag_optim_dom==1 | flag_optim_dom==2);
        resp_prob=impacts(optdom,12);
        %problemi cardiovascolari
        impacts(:,13)=pop.*full_aqi.*extoth(13,1);
        card_prob=impacts(optdom,13);
        %yoll
        ...
        yoll_prob=impacts(optdom,14)./(pop(optdom).*pop30_90(optdom))*12*60; %in monhts, over 60 years of life
